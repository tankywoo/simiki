...
class InitSite(object):
    ...
    def __init__(self, config_file, target_path):
        ...
        try:
            ...
            logging.error(str(e))
        self.source_path = osp.dirname(__file__)
    ...
    def get_config_file(self):
        dst_config_file = osp.join(self.target_path, "_config.yml")
    ...
    def get_fabfile(self):
        ...
        dst_fabfile = osp.join(self.target_path, "fabfile.py")
        self.get_file(src_fabfile, dst_fabfile)

    def get_first_page(self):
        nohidden_dir = listdir_nohidden(osp.join(self.target_path, "content"))
    ...
    def get_default_theme(self, theme_path):
        src_theme = osp.join(self.source_path, "themes")
    ...
    def init_site(self):
        content_path = osp.join(self.target_path, self.configs["source"])
        output_path = osp.join(self.target_path, self.configs["destination"])
        theme_path = osp.join(self.target_path, "themes")
